...
def parse_module_defs(module_defs):
    ...
    for name, module in module_defs.named_modules():
        #print('bbb',module_defs.state_dict()[name])
        #if zifu in shuju:
        print("aaaaa", name, module)
        if isinstance(name, nn.BatchNorm2d):
            print("aaaaa",name)
        #print("aaaaa",module_defs.state_dict()[name])
        #print("aaaaa",i)
    ...
    def updateBN(sr_flag, module_list, s, prune_idx, epoch, idx2mask=None, opt=None):
        if sr_flag:
            ...
            for idx in prune_idx:
                ...
                ########torch.sign(input, out=None) 说明：符号函数，返回一个新张量，包含输入input张量每个元素的正负（大于0的元素对应1，小于0的元素对应-1，0还是0）
            if idx2mask:
                for idx in idx2mask:
                    bn_module = module_list[idx][1]
